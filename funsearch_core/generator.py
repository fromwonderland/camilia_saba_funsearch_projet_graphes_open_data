...
class FunSearchGenerator:
    def __init__(
        ...
        *,
        ...
        self.ollama_model = ollama_model or "deepseek-coder-v2"  
    ...
    def _generate_with_ollama(
        ...
        """
        system = (
            "You are an expert graph coloring algorithm designer. "
            "You generate ONLY valid Python code (no markdown fences). "
            "Return exactly one function definition color_graph(G) that returns {node: color} dict. "
            "CRITICAL: Never return None when graph has nodes - this causes errors in the evaluator."
        )
        reinject = ""
        # Réactiver reinject pour DeepSeek Coder V2 (plus créatif)
        ...
        user_prompt = (
            self.prompt
            + "\n\nGraph format: G is a NetworkX Graph object with nodes and edges."
            + "\nReturn {node: color} dictionary mapping each node to a color integer."
            + "\nCRITICAL SAFETY: Always return a valid dictionary when graph has nodes. Never return None."
            + reinject
            + "\n\nNow output Python code only, defining color_graph(G) and helper functions."
        )
        ...
        for i in range(n):
            payload = {
                ...
                "stream": False,
                "options": {
                    "temperature": 1.2 if i == 0 else 1.5,  # Optimisé pour DeepSeek Coder V2
                    "top_p": 0.9,
                    "num_predict": 512,
                },
            }
