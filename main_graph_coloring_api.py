"""
Main FunSearch script for Graph Coloring heuristics evolution using API LLMs
"""
...
from funsearch_core.generator_api import FunSearchAPIGenerator


class FunSearchGraphColoringAPI:
    """
    Main class for FunSearch Graph Coloring evolution using API LLMs
    """
    
    def __init__(self, max_cycles: int = 5, candidates_per_cycle: int = 3, 
                 provider: str = "mistral", api_key: str = None):
        """
        Initialize FunSearch for Graph Coloring with API.
        ...
        print(f"🎨 FunSearch Graph Coloring API Initialisé")
...
'''
    ...
    def run_evolution(self) -> None:
        """Run complete evolution process using API"""
        print("=" * 60)
        print("🚀 DÉMARRAGE DE L'ÉVOLUTION FUNSEARCH API")
        print("=" * 60)
        ...
        try:
            ...
            generator = FunSearchAPIGenerator(prompt_template_file, self.provider)
        ...
        for cycle in range(1, self.max_cycles + 1):
            ...
            results = evaluate_all_heuristics(
                ...
                max_graphs=30  # Réduit pour tests API plus rapides
            )
        ...
        print("\n🎉 ÉVOLUTION TERMINÉE !")
        print(f"📊 Rapport final sauvegardé dans {self.logs_dir}/final_report_api.json")
        print(f"📈 Graphiques sauvegardés dans {self.graphs_dir}/evolution_summary_api.png")
    
    def _save_cycle_results(self, cycle: int, results: List[Dict], rankings: List[Dict]) -> None:
        """Save results for a specific cycle"""
        cycle_file = self.logs_dir / f"cycle_{cycle}_api.json"
    ...
    def _generate_new_heuristics(self, rankings: List[Dict], generator: FunSearchAPIGenerator) -> None:
        ...
        for i, heuristic_code in enumerate(new_heuristics):
            try:
                ...
                print(f"  ❌ Erreur sauvegarde heuristique {i+1}: {e}")
    ...
    def _generate_final_report(self) -> None:
        ...
        report_file = self.logs_dir / "final_report_api.json"
    ...
    def _create_evolution_plots(self) -> None:
        ...
        ax1.set_title(f'Évolution du Meilleur Score ({self.provider.upper()})')
        ...
        ax2.set_title(f'Évolution du Score Moyen ({self.provider.upper()})')
        ...
        plt.tight_layout()
        plot_file = self.graphs_dir / "evolution_summary_api.png"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="FunSearch Graph Coloring Evolution with API")
    ...
    # Run FunSearch
    funsearch = FunSearchGraphColoringAPI(
